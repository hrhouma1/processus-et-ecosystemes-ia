...
>
> Tu vas :
> 1. Construire et démarrer un serveur MLflow **100 % dans Docker** (aucun Python sur l'hôte).
> 2. Ouvrir l'UI MLflow sur `http://localhost:5000`.
> 3. Créer **un seul run minimal** (`mlflow.start_run` + `mlflow.log_metric("foo", 1)`) directement depuis le conteneur avec `docker compose exec`.
> 4. Voir ce run apparaître dans l'UI et comprendre où sont stockés la **base SQLite** (`./database/mlflow.db`) et les **artefacts** (`./mlruns/`) grâce aux bind mounts.
>
> À la fin, tu sais démarrer un MLflow propre, l'arrêter avec `docker compose down`, et tu reconnais la pile `mlflow + bind mounts + SQLite` qui sert de base à tous les chapitres suivants.
